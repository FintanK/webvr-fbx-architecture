...
export class CreateComponent implements OnInit, AfterViewInit {
  ...
  private createScene() {
    ...
    this.loaderGLTF.load('assets/models/pagoda.glb', (gltf: GLTF) => {
      this.model = gltf.scene;
      const box = new THREE.Box3().setFromObject(this.model);
      box.getCenter(this.model.position); // this re-sets the mesh position
      this.model.position.multiplyScalar(-1);
      // this.model.position.setX(5);
    ...
    const box = new THREE.Box3().setFromObject(this.sprite);
 ...
 */
  ...
  updateScreenPosition() {
    ...
    this.annotation.style.top = `${vector && vector.y ? vector.y : 0}px`;
    this.annotation.style.left = `${vector && vector.x}px`;
